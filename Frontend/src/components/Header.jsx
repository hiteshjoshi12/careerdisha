  ...
  return (
    ...
    <header className="top-0 left-0 w-full z-50 font-sans">
      ...
      <div className="hidden lg:flex bg-black text-gray-400 py-1.5 px-6 lg:px-12 justify-end items-center gap-8 text-[11px] font-semibold tracking-wider border-b border-gray-900">
        <a
          href="mailto:careerdisha@edumilestone.com"
      ...
      <nav className="px-6 lg:px-12 py-3 flex justify-between items-center">
        ...
        <div className="hidden lg:flex items-center gap-8 font-semibold text-[14px] text-gray-300">
          <Link
            to="/profiling"
            className="hover:text-orange-500 transition-colors uppercase tracking-wide"
          >
            Profiling
          </Link>
          ...
          >
            <button className="flex items-center gap-1 hover:text-orange-500 py-4 transition-colors uppercase tracking-wide">
              ...
              <ChevronDown
                size={14}
                className={`transition-transform duration-300 ${isTravelOpen ? "rotate-180" : ""}`}
              />
            </button>
            ...
            <AnimatePresence>
              {isTravelOpen && (
                ...
                >
                  {["Flights Booking", "Hotels Booking"].map((item) => (
                    <a
                      key={item}
                      href="/travel"
                      className="block px-6 py-2.5 text-gray-400 hover:bg-orange-600/10 hover:text-orange-500 transition-colors text-[13px]"
                    >
                      {item}
                    </a>
                  ))}
            ...
            >
              ...
              <div className="flex flex-col gap-6 font-bold text-lg text-gray-300">
                ...
                <div className="space-y-4">
                  <p className="text-xs uppercase text-gray-500 tracking-widest font-black">
                    Travel Arrangements
                  </p>
                  <div className="flex flex-col gap-3 pl-4 border-l-2 border-orange-600">
                    <a href="/travel" className="text-base font-medium">
                      Flights Booking
                    </a>
                    <a href="/travel" className="text-base font-medium">
                      Hotels Booking
                ...
                {showDashboardButtons ? (
                  <div className="flex flex-col gap-4 mt-4">
                    <Link
                      to="/"
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="text-center py-4 border border-gray-800 rounded-xl"
                    >
                      Home
                    </Link>
                    <Link to="/login">
